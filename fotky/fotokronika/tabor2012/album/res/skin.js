...
(function($) {
	...
	******************************************************** */
		...
		return this.each( function() {
			...
			var setupThumbs = function( t ) {
				...
				images.each( function(n) {
					...
					im.data(id.src, l.replace(/\.(php|html*)/, '.' + im.data(id.ext)));
			...
			controls.on({
				...
				mouseleave: function() { 
					cmo = false;
					$(this).stop(true, false).fadeTo(200, settings.controlbarOpacity);
				}
			});
	...
	$.fn.turtleGallery.defaults = {
		...
		controlbarOpacity: 0.7,		// opacity of control bar when the mouse is not over
